...
const BuyerDashboard = (props) => {
  ...
  useEffect(() => {
    ...
    axios 
      ...
      .then((response) => {
        ...
        axios.post("http://localhost:4000/buyer/getWallet",Buyer).then((res) => {
        ...
        axios.get("http://localhost:4000/vendor").then((response) => {
            ...
            for(let i = 0 ; Vendors[i] != undefined ; i++)
            {
                ...
                console.log(Number(closeTime[0]));
                console.log(Number(openTime[0]));
  ...
  const manageFavs = (ind) => {
    ...
    else
    {
        ...
        axios
            .post("http://localhost:4000/buyer/addToFav", User)
            .then((response) => {
  ...
  const updateBalance = () => {
    ...
    axios
        .post("http://localhost:4000/buyer/addToWallet", User)
        .then((response) => {
            const user = JSON.parse(localStorage.getItem("user"));
            user.balance += amount;
            localStorage.setItem("user", JSON.stringify(user));
  ...
  const removeFromFavs = (ind) => {
    ...
    axios
        .post("http://localhost:4000/buyer/removeFromFav", User)
        .then((response) => {
            const user = JSON.parse(localStorage.getItem("user"));
            let index = 0;
  ...
  const placeOrder = () => {
    ...
    else
    {
        ...
        axios
            .post("http://localhost:4000/order/place", Order)
            .then((response) => {
                setIsBuy(false);
                setQuantity(1);
            })
        axios
            .post("http://localhost:4000/buyer/updateWallet", Wallet)
            .then((response) => {
